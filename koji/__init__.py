...
class NetworkError(GenericError):
    """Raised when network is broken"""
    faultCode = 1023

class APIVersionError(GenericError):
    """Raised when API version unmatched between client and server fails"""
    faultCode = 1024
...
class ClientSession(object):
    ...
    def activate(self, options):
        ...
        Args: options generated by optparse
        Returns: None, if success
        Raises: AuthError, if auth failed, error.args[0] is the debug trace for client
                ProtocolError, if network broken
                APIVersionError, if API version unmatched between client and server
        """
        create_auth_handler(options).login(self, options)
        ...
        try:
            ...
            if ret != API_VERSION:
                raise APIVersionError(
                    "Warning: The server is at API version %d and the client is at %d" % (ret, API_VERSION))
        except xmlrpclib.ProtocolError as e:
            raise NetworkError("Error: Unable to connect to server", e)
...
class ClientAuthHandler(object):
    ...
    def login(self, session, options):
        """
...
class DefaultAuthHandler(ClientAuthHandler):
    ...
    def login(self, session, options):
        ...
        for (authtype, handler) in self.handler_seq:
            try:
                if handler.login(session, options):
        ...
        raise AuthError(self._build_debug_msg(session, options))

    def _build_debug_msg(self, session, options):
        trace = ""
        for i, (key, info) in enumerate(self.infos):
            trace += "    - [%d] Bad Authentication via %s:\n" % (i, key)
            items = sorted([(name, value) if value is None or str(value).strip() else (name, '<EMPTY>') for name, value in info.iteritems()])
...
or use "koji --authtype=noauth <command> ..." to skip login phase.
    --authtype=AUTHTYPE   force use of a type of authentication, options:
                          %s)
""" % (session.baseurl, trace, options.profile, ', '.join(sorted(_handler_mapping.keys())))
        return self.info
